...
function applyAuthInCookie(req) {
  const {
    reduxStore: store,
    cookies,
  } = req;
  // req.cookies is a object
  debug('cookies.token', cookies.token);
  const tokenInCookies = cookies.token;
  store.dispatch(action.setTokenInCookies(tokenInCookies));
  return req;
}
...
export default function renderer({ clientStats }) {
  return (req, res) => {
    applyRouteCheckResult(applyAuthInCookie(applyInitStore(req))).then(() => {
